...
void can_send_callback(const geometry_msgs::Twist::ConstPtr &twist,CANHelper *canHelper)
{
    CHR10MEV_VCU data;
    ...
    {
        msg.forward = 1;
  	    msg.velocity = twist->linear.x *3.6;
        cout<<"msg.velocity:"<<msg.velocity<<endl;
        msg.driverless_enabled = 1;
        msg.park = 0;
    ...
    //if(twist->angular.z <= 0.15 && twist->angular.z >= 0.15 )
    ...
    {
        msg.angle = 900.0 - (twist->angular.z/3.1416)*1700.0;
	    msg.left = 1;
            if(twist->angular.z > 0.3)
	    {
		// twist->angular.z=0.3;
	    }
	    //msg.angle_velocity =  (twist->angular.z * 2.0);  //9.55  1rad/s = 9.55rpm  转16进制可能         
            msg.angle_velocity =  4.0;
    }else if(twist->angular.z < 0.0)
    {
	if(twist->angular.z < -0.3)
	    {
	    ...
	    msg.right = 1;
        msg.angle = 900 - (twist->angular.z/3.1416)*1700.0; 
	    //msg.angle_velocity =  (-twist->angular.z * 2.0);
        msg.angle_velocity =  4.0;
    }
    ...
    data.velocity =  0xff & (int)(msg.velocity *51.0);
    //data.velocity =  0xff & msg->velocity ;
    data.angle = (unsigned short)(msg.angle);
    //data.angle = 0xffff &msg.angle;
    data.angle_velocity =  0xff & (int)(msg.angle_velocity*224.0)/10;
    //data.angle_velocity =  0xff & (int)msg->angle_velocity;
...
{
    CANHelper canHelper(0,0);
    canHelper.openCAN();
    ros::init(argc, argv, "can_node");
    ros::NodeHandle nh;
    //ros::NodeHandle n("~");
    ros::Publisher can_rec_pub = nh.advertise<synchronizer_laser_imu::can_msg>("can_rec",2);
    //ros::Subscriber sub=nh.subscribe<synchronizer_laser_imu::can_msg>("can_send2",1,boost::bind(&can_callback, _1, &canHelper));
    ros::Subscriber sub = nh.subscribe<geometry_msgs::Twist>("cmd_vel",1, boost::bind(&can_send_callback, _1, &canHelper));
