...
void CANHelper::printCHR10MEV_VCU(const CHR10MEV_VCU* data)
{
    ...
    cout << " Value: " <<  dec 
        ...
        << (unsigned int)data->mode << " " << (unsigned int)data->enable << " "
        << (unsigned int)data->speed_enable << " " << (unsigned int) data->driverless << " "
        << (unsigned int)data->driverless_enabled << " " << (unsigned int) data->charge << endl;

}

void CANHelper::printVCI_CAN_OBJ(const VCI_CAN_OBJ* data)  //该函数接收data并打印
{
...
int CANHelper::openCAN()
{
    ...
    {  
        ...
        config.Timing0=timing0;/*波特率250 Kbps  0x01  0x1C*/
        ...
        {
             cout << "Not start send CAN " << sendCANId << " or send CAN duplicates" << endl;
        }
...
void CANHelper::recvMultiMsgs(CHR10MEV_VCU* data,int *len)
{
    ...
    { 
        ...
        {             
            ...
            int ret = recvData2Chr10mev(i, data+msglen);
            
            cout << " Receive message identifier: " << (data+msglen)->identifier << endl;
            if (ret > 0)
            {
                cout << "RX " ;
                printCHR10MEV_VCU(data+msglen);
                ++msglen;
            }
...
void CANHelper::chr10mev2SendData(const CHR10MEV_VCU* data, int index)
{
    ...
    if (data->identifier == 0x18909C11)//发送
    {
        //AI_VCU
...
int CANHelper::recvData2Chr10mev(int index, CHR10MEV_VCU* data)
{
    int ret = 0;
    data->identifier = recvMsgArray[index].ID;
    if (data->identifier == 0x1890119C)  //接收
