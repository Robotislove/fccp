...
public:
  ...
  void readFrom(int fd)
  {
    ...
    else
    {
      ...
      ROS_DEBUG("Buffer read() returned error.");
    }
  }
  ...
  char* getNextBuffer()
  {
    ...
    if (start_of_message == NULL)
    {
      ...
      ROS_DEBUG("No STX found, dropping %d bytes from buffer.", total_length_);
      total_length_ = 0;
    }
    else if (buffer_ != start_of_message)
    {
      // Start of message found, ahead of the start of buffer. Therefore shift the buffer back.
      ROS_DEBUG("Shifting buffer, dropping %d bytes, %d bytes remain.",
